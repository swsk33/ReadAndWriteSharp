...
namespace Swsk33.ReadAndWriteSharp
{
	...
	public class TextFileWriter
	{
		...
		public static bool ReplaceSpecificLine(String filePath, string content, int line, Encoding encoding)
		{
			...
			writer.Write(text);
			writer.Close();
			if (TextFileReader.ReadSpecificLine(filePath, line, encoding).Equals(content))
			{
				success = true;
			}
			return success;
		...
		public static bool AppendText(string filePath, string content, Encoding encoding)
		{
			...
			writer.WriteLine(content);
			writer.Close();
			if (TextFileReader.ReadSpecificLine(filePath, TextFileReader.GetFileLine(filePath), encoding).Equals(originContent))
			{
				success = true;
			}
			return success;
		...
		public static bool InsertText(string filePath, string content, int line, Encoding encoding)
		{
			...
			writer.Write(text);
			writer.Close();
			if (TextFileReader.ReadSpecificLine(filePath, line + 1, encoding).Equals(content))
			{
				success = true;
			}
			return success;
		...
		public static bool ClearAll(string filePath)
		{
			...
			if (TextFileReader.GetFileLine(filePath) == 0)
			{
				success = true;
			}
			return success;
